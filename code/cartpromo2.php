...
<?php 
        ...
        if ($row['foodname'] == 'Pineapple Fried Rice'){
            $items[$i]=$row['foodname'];
            $prices[$i]=$row['amount'];
            $discount[$i] = $row['amount'];
        }else {
            $items[$i]=$row['foodname'];
            $prices[$i]=$row['amount'];
        }
...
for ($i=0; $i < count($_SESSION['cart']); $i++){
    ...
    $discountedprice = 0.25 * $discount[$_SESSION['cart'][$i]];
    $total = $total - $discountedprice;
	$disc = $disc + $discountedprice;
}
...
//https://www.upgrad.com/blog/implode-in-php/
$ids = implode(', ', $_SESSION['cart']);
//https://stackoverflow.com/questions/5803472/sql-where-id-in-id1-id2-idn
//SELECT * FROM TABLE WHERE foodid = 1 OR foodid = 2 OR ... OR foodid = n
$query = "SELECT * FROM food_menu where foodid IN ($ids)";
$result = $conn->query($query);
$num_results = $result->num_rows; 
...
for ($i=0; $i < $num_results; $i++){ 
    $row = $result->fetch_assoc(); // row = row.next()
	echo "<tr>";
	//echo "<td>" .$row['foodid']. "</td>";
    echo "<td>" .$row['foodname']. "</td>";
	echo "<td align='right' class='price' value ='".$row['amount']."'>$";
	echo $row['amount']. "</td>"; //2 is 2 decimal places 
...
?>
	...
	<tfoot>
    <tr>
        <th align='right'>Discount(25% off fried rice):  </th><br>
        <th>-$<?php echo number_format($disc, 2); ?></th>
	...
	<tr>
	<td><a><i>Promo code:'makmakmaster' applied.</i></a></td>
	</tr>
	</tfoot>
